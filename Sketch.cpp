...
void draw(obj M, sf::RenderWindow& window, Camera cam, Vec4 light) {
	...
	for (triangle t : M.triangles) { ///detecting backface
		Vec4 v1 = t.p1 - t.p0;
		Vec4 v2 = t.p2 - t.p0;
		...
		Vector<4> pp0 = proj(Transform * t.p0);
		Vector<4> pp1 = proj(Transform * t.p1);
		Vector<4> pp2 = proj(Transform * t.p2);

		if (std::min({ pp0[2], pp1[2], pp2[2] }) < 0.5)continue; //inorder to not render the pixel with z value more than 0.5 else stuff behind camera will also be visible
		...
		for (int i = max(0, ymin); i <= min(ymax, SCREEN_HEIGHT - 1); i++) {//y axis
